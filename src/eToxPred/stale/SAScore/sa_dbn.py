...
class DBN(object):
    ...
    def __init__(self, numpy_rng, theano_rng=None, n_ins=1024,
        ...
        # the data is presented as a vector
        self.x = T.matrix('x')
        ...
        # We now need to add a linear regression layer on top of the MLP
        self.linearLayer = SemiLastLayer(
        ...
        # be small
        self.L1 = abs(self.linearLayer.W).sum()
        for i in range(2 * self.n_layers)[0::2]:
            self.L1 += abs(self.params[i]).sum()

        # square of L2 norm ; one regularization option is to enforce
        # square of L2 norm to be small
        self.L2_sqr = (self.linearLayer.W ** 2).sum()
        for i in range(2 * self.n_layers)[0::2]:
            self.L2_sqr += (self.params[i] ** 2).sum()

        # compute the cost for second phase of training, defined as the
        # sqaured error of the linear regression (output) layer
        self.finetune_cost = self.linearLayer.squared_errors(self.y).max()+ 0.00001 * self.L1 + 0.0001 * self.L2_sqr

        # compute the gradients with respect to the model parameters
        # symbolic variable that points to the number of errors made on the
        # minibatch given by self.x and self.y
        self.errors = self.linearLayer.squared_errors(self.y)
